...
namespace nes::app
{
	// Implementation copied from: https://github.com/kokke/tiny-AES-c/tree/master
...
#define AES_keyExpSize 176
		...
		void AES_init_ctx(AES_ctx* ctx, u8 const* key);
		...
		void AES_CBC_encrypt_buffer(AES_ctx* ctx, u8* buf, u32 length);
		void AES_CBC_decrypt_buffer(AES_ctx* ctx, u8* buf, u32 length);
		...
		//        no IV should ever be reused with the same key
		void AES_CTR_xcrypt_buffer(AES_ctx* ctx, u8* buf, u32 length);
...
#define MULTIPLY_AS_A_FUNCTION 0
		...
		u8 const Rcon[11] = { 0x8d, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36 };
...
#define getSBoxValue(num) (sbox[(num)])
		...
		void KeyExpansion(u8* RoundKey, u8 const* Key)
		{
			unsigned i, j, k;
		...
		void AddRoundKey(u8 round, state_t* state, u8 const* RoundKey)
		{
			for (u8 i = 0; i < 4; ++i)
			{
				for (u8 j = 0; j < 4; ++j)
...
#define getSBoxInvert(num) (rsbox[(num)])
		...
		void Cipher(state_t* state, u8 const* RoundKey)
		{
			u8 round = 0;
...
#if (defined(CBC) && CBC == 1) || (defined(ECB) && ECB == 1)
		void InvCipher(state_t* state, const u8* RoundKey)
		{
			u8 round = 0;
...
/*****************************************************************************/
		...
		void XorWithIv(u8* buf, const u8* Iv)
		{
			u8 i;
		...
		void AES_CBC_decrypt_buffer(AES_ctx* ctx, u8* buf, u32 length)
		{
			...
			for (i = 0; i < length; i += AES_BLOCKLEN)
			{
				copy(buf, storeNextIv, AES_BLOCKLEN);
				InvCipher((state_t*)buf, ctx->RoundKey);
				XorWithIv(buf, ctx->Iv);
				copy(storeNextIv, ctx->Iv, AES_BLOCKLEN);
				buf += AES_BLOCKLEN;
			}
		}
		...
		void AES_CTR_xcrypt_buffer(AES_ctx* ctx, u8* buf, u32 length)
		{
			...
			for (i = 0, bi = AES_BLOCKLEN; i < length; ++i, ++bi)
			{
				...
				{
					copy(ctx->Iv, buffer, AES_BLOCKLEN);
	...
	auto aes256::encrypt(span<u8> const buffer, key const key) -> void
	{
		auto ctx = AES_ctx{};
		AES_init_ctx(&ctx, key.get_data());
		AES_CTR_xcrypt_buffer(&ctx, buffer.get_data(), buffer.get_length());
	}

	auto aes256::decrypt(span<u8> const buffer, key const key) -> void
	{
		// For CTR: Encryption is same as decryption.
